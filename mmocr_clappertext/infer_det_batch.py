"""Run text detection over per-video image folders with MMOCR."""
...
def batch_infer(
    input_dir, output_dir, model, weights=None, device="cuda", batch_size=16
):
    input_dir = Path(input_dir)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    inferencer = MMOCRInferencer(det=str(model), det_weights=weights, device=device)
    ...
    for video_dir in tqdm(video_dirs, desc=Path(str(model)).stem):
        ...
        with output_path.open("w", encoding="utf-8") as output_file:
            for prediction, image_path in zip(result["predictions"], images):
                record = {"filename": image_path, "detections": prediction}
