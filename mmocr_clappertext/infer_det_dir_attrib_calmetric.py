"""Evaluate ClapperText detection by handwriting and occlusion attributes."""
...
def load_ground_truths_per_video(gt_dir: str) -> Dict:
    ...
    for p in Path(gt_dir).glob("*.json"):
        ...
        for ln in Path(p).read_text(encoding="utf-8").splitlines():
            ...
            img = d["file_name"]
            poly_attr_list = []
            for box in d["gt_bboxes"]:
                poly = Polygon(box["polygon"])
                poly_attr_list.append(
                    (
                        poly,
                        {
                            "handwritten": bool(box.get("handwritten", False)),
                            "occluded": bool(box.get("occluded", False)),
                        },
                    )
                )
            video_gt[vid][img] = poly_attr_list
    return video_gt
...
def evaluate_single_video(
    gt_dict, pred_dict, iou_thr=0.5, score_thr=0.5, attr_name=None, attr_value=None
):
    tot_gt = tot_pred = tot_tp = 0
    for img, gt_pairs in gt_dict.items():
        gt_polys = [
            p
            for p, attr in gt_pairs
            if (attr_name is None) or (attr.get(attr_name) == attr_value)
        ]
        ...
        for i, g in enumerate(gt_polys):
            for j, pr in enumerate(pred_polys):
                iou_mat[i, j] = poly_iou(g, pr)
...
def evaluate_dataset(gt_dir, pred_dir, score_thr: float):
    ...
    subsets = [
        ...
        ("printed", "handwritten", False),
        ("occluded", "occluded", True),
        ("non_occluded", "occluded", False),
    ]

    results = {}
    for name, attr_name, attr_val in subsets:
        res = {}
...
def get_deploy_stats(det_cfg_path, det_weights, device="cuda"):
    ...
    stats.update({"input_h": int(H), "input_w": int(W)})
...
def run_experiment(
    ...
    if mode == "zeroshot":
        ...
        metric_json = exp_root / "evaluation_results_zeroshot.json"
        ...
        metric_json = exp_root / "evaluation_results_finetune.json"
...
def parse_args():
    ...
    parser.add_argument("--summary", default="evaluation_results_attributes.json")
